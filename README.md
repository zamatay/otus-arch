...
### Что сделано
   ...
   - Создал новую таблицу user_reade для хранения информации кто прочитал сообщение
   - Добавил новый метод Read и реализовал его как в pg так и в redis хранилищах
   - Создал новый сервис Counter который по grpc получает данные post_id и инкрементит в редис счетчик или дикрементит
   - При вызове метода Read так же посылается в новый сервис что надо проинкрементить по данному post_id счетчик, тем самым мы реализовали патерн saga (хореография) без единого центра, что с маленьким количеством сервисов предпочтительней
   - в сервисе Counter так же реализовал по rest получение данного счетчика по указанному post_id
### Во избежании рассинхрона 
   - можно реализовать не инкремент и дикремент счетчика а просто его установку, уже выполнил подсчет во время вызова методв Read, но пока оставил так, в дальнейшем осталось прост реализовать метод установки счетчика. 
...
- Добавлены в docker-compose сервисы zabbix-server, zabbix-db, zabbix-web
- На локальной машине установлен агент zabbix
- В приложении добавлен endpoint /metrics который отдает метрики приложения
- Настроен сбор метрик c приложения в prometheus
- Добавлен сбор метрик для всех входящих соединений, через middleware (http_request_duration_seconds_count, http_request_duration_seconds_bucket)
- В графану выведен дашбоард по RPS и квантиль 0.99
- В графана установлен плагин для работы с zabbix, выведены dashboard с локальной машины, по cpu и средней загрузке системы 
- В графана добавлены метрики с источника prometheus который собирает данные с приложения
